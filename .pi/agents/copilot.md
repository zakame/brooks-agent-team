...
<SUBAGENT-STOP>
You are already in the Copilot role. Do not invoke `using-brooks-team`, `surgeon`, `assemble-team`, `assemble-with-fleet`, `assemble-with-grok-team`, `assemble-with-hermes-team`, or `assemble-with-pi-team` skills.
</SUBAGENT-STOP>
...
> **Note:** This agent has no `bash` access, so it cannot run `git diff`/`git log` itself. The dispatcher must include the relevant diff or file contents directly in the prompt.
