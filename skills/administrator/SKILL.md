...
On Hermes Agent, this is the kanban board (`kanban_create`/`kanban_list`), but only when the current profile has the `kanban` toolset explicitly enabled — a regular session has no `kanban_*` tools by default. Without it, track state the same way as on Codex or OpenCode: no dedicated tool, just an explicit plan carried in context.
