...
description: |
  ...
  may not apply to the current language or runtime. The Language Lawyer cites the spec,
  not the assumption.
  Examples:
  <example>
  Context: Surgeon is unsure whether async behavior is consistent across runtime versions.
  user: "Does this Promise resolution order hold in Node 18 and 20?"
  assistant: "I'll dispatch the Language Lawyer to verify the spec and runtime behavior before we rely on it."
  </example>
  <example>
  Context: A deprecation warning appeared with an unclear migration path.
  user: "We're getting a deprecation warning on this API but the docs are vague"
