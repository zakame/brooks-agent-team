...
- **Pi Coding Agent support**: pi-coding-agent (`@earendil-works/pi-coding-agent`, pre-1.0, verified against v0.87.1) natively supports the Agent Skills standard and reads `AGENTS.md`/`CLAUDE.md` as context files (not trust-gated), so no new adapter files are needed via the existing `.agents/skills/` mirror — but project-local `.agents/skills/` still needs the same one-time project-trust decision as Pi's own `.pi/skills/` (confirmed by reading `hasTrustRequiringProjectResources()` in `trust-manager.ts`; only the global `~/.agents/skills/` is unconditionally exempt). Direct inspection of Pi's extension API confirmed core has no subagent/delegate tool and no shared task list at all; the only parallel-work mechanism is the official but opt-in `examples/extensions/subagent/` example extension, which spawns separate OS processes per task and defines its own per-role agent-definition format (`.pi/agents/*.md`, project-scope; `~/.pi/agent/agents/*.md`, user-scope). New `.pi/agents/copilot.md`, `tester.md`, and `language-lawyer.md` in that format (inert unless a user has the extension installed and requests project scope), new `assemble-with-pi-team` skill documenting the extension-gated parallel workflow with a fallback to single-session `assemble-team`, and README/AGENTS.md/`assemble-team`/`administrator` updated accordingly.
...
- **Pi Coding Agent subagent extension: documented the no-clone shortcut**: the `examples/extensions/subagent/` extension's own README installs from a fresh clone of the `earendil-works/pi` monorepo, but `examples/` ships inside the same `@earendil-works/pi-coding-agent` package a user already has installed (confirmed via `package.json`'s `files` array and the actual `pi.dev` curl installer script, across plain npm installs and both curl-installer modes). README now documents locating that existing install instead — `mise where pi` for mise (user-verified: `$(mise where pi)/pi/examples/extensions/subagent`), or `readlink -f "$(command -v pi)"` two directories up from `dist/bundle/cli.js` for plain npm/curl installs — plus a caution about unrelated, unofficial "pi-subagent"-named npm packages. Also documented, confirmed at the `package-manager.ts` source level: `pi install <path>/examples/extensions/subagent` is not a viable alternative to manual symlinking — it silently half-installs, registering the sibling `prompts/` workflow templates as slash commands while never registering `index.ts`/`agents.ts` (the actual `subagent` tool) as an extension at all, since package discovery only checks for the presence of `extensions/`/`skills/`/`prompts/`/`themes/` at the source root and `subagent/` has the latter but not the former. No automation was added — other shim-based version managers besides mise remain unverified, so this stays manual copy-paste guidance. Also documents a minimal install: only `index.ts`/`agents.ts` need symlinking into `~/.pi/agent/extensions/subagent/` to use this repo's own `.pi/agents/*.md` roles — the extension's sample agents/prompts (flat `~/.pi/agent/agents/`/`prompts/`) are skippable, confirmed at the source level (the tool's `agentName` parameter is validated at call time against a fresh scan of whichever `agentScope` is passed, never a fixed enum computed from `~/.pi/agent/agents/` at load time, and a missing agents/prompts directory degrades to an empty result rather than erroring). Also documents the user-scope equivalent of the global skills symlink: symlinking `.pi/agents/*.md` into `~/.pi/agent/agents/` makes these roles available from any project with no `agentScope` parameter needed, since `"user"` is the documented default. `assemble-with-pi-team` updated to mention this alternative. Also confirmed and documented that file-based `.pi/agents/*.md` is the *only* mechanism this tool offers for defining a role's system prompt — its full parameter schema (`SubagentParams`/`TaskItem`/`ChainItem`) has no inline instructions/system-prompt field, unlike Hermes' `delegate_task`, so a Hermes-style dynamic-context-pasting approach genuinely isn't available here.

- **Pi Coding Agent skills guidance corrected and improved**: the initial Pi Coding Agent write-up implied a project-local `.agents/skills/` (cloning this repo and working from inside it, or copying into a target project) was the only path, requiring a trust prompt per project. Follow-up research into the actual source (`package-manager.ts`'s resource-assembly order, `resource-loader.ts`) found a better path: the global `~/.agents/skills/` is scanned unconditionally in every project — structurally outside the trust-gated code branch, not merely exempted from the trust check — so a one-time `ln -sf .../skills ~/.agents/skills/brooks-agent-team` makes every skill available everywhere, permanently, with no trust prompt ever. The `--skill <path>` CLI flag was also confirmed to bypass project trust entirely for one-off sessions (a CLI-supplied path never calls `isProjectTrusted()`). README, AGENTS.md, and `assemble-with-pi-team` updated to lead with the global-symlink approach, keeping the project-local/trust-gated path as a documented alternative.
